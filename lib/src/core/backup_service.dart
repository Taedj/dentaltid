...
class BackupService {
  ...
  Future<String?> createBackup({bool uploadToFirebase = false}) async {
    try {
      ...
      archive.addFile(ArchiveFile('dentaltid.db', databaseFile.lengthSync(), databaseFile.readAsBytesSync()));

      final backupFileName = 'dentaltid_backup_${DateTime.now().millisecondsSinceEpoch}.zip';

      final tempDir = await Directory.systemTemp.createTemp();
      final tempFilePath = join(tempDir.path, backupFileName);
      final output = File(tempFilePath).openWrite();
      output.writeAll(encoder.encode(archive));
      await output.close();

      if (uploadToFirebase) {
        final backupId = await _firebaseService.uploadBackupToFirestore(tempFilePath);
        ...
        await File(tempFilePath).copy(outputFile);
        await tempDir.delete(recursive: true);
  ...
  Future<bool> restoreBackup({String? backupId}) async {
    try {
      ...
      if (backupId != null) {
        ...
        final downloadedFile = await _firebaseService.downloadBackupFromFirestore(backupId, tempFilePath);
