...
class DeveloperService {
  ...
  Stream<List<ActivationCodeModel>> getActivationCodes() {
      return _firestore.collection(_codesCollection).orderBy('createdAt', descending: true).snapshots().map((snapshot) {
          return snapshot.docs.map((doc) => ActivationCodeModel.fromFirestore(doc)).toList();
      });
  }
