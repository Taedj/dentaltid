...
class _DeveloperDashboardScreenState
    extends ConsumerState<DeveloperDashboardScreen> with SingleTickerProviderStateMixin {
...
class UserManagementTab extends StatelessWidget {
             ...
             Container(
                 ...
                 child: Row(
                     ...
                     children: [
                         ...
                         _StatCard('Premium', premiumUsers.toString(), Icons.star, Colors.amber),
                         _StatCard('Active Trials', trialUsers.toString(), Icons.timer, Colors.orange),
                         _StatCard('Est. Monthly', '\$${revenueEstimate}0', Icons.attach_money, Colors.green),
                     ],
                 ),
             ),
                ...
                itemBuilder: (context, index) {
                  ...
                  if (user.isPremium) {
                      if (user.premiumExpiryDate != null) {
                          final days = user.premiumExpiryDate!.difference(DateTime.now()).inDays;
                       ...
                       final daysUsed = DateTime.now().difference(user.trialStartDate!).inDays;
                       final days = 30 - daysUsed;
                       daysLeftInfo = days > 0 ? '$days days left (Trial)' : 'Trial Expired';
                  ...
                  return Card(
                    ...
                    child: ExpansionTile(
                      leading: CircleAvatar(
                        ...
                        child: Icon(user.role == UserRole.developer ? Icons.code : Icons.person),
                      ),
                      title: Text(user.dentistName ?? user.email),
                      subtitle: Text(user.phoneNumber ?? 'No Phone'),
                      trailing: Chip(
                        label: Text(user.plan.toString().split('.').last.toUpperCase()),
                          ...
                          Padding(
                              ...
                              child: Column(
                                  ...
                                  children: [
                                      ...
                                      const Divider(),
                                      _InfoRow('Status', user.status.toString().split('.').last),
                                      _InfoRow('Time Left', daysLeftInfo, color: daysLeftColor),
                                      ...
                                      Wrap(
                                          ...
                                          children: [
                                                ElevatedButton.icon(
                                                  onPressed: () => _showPlanDialog(context, user, developerService),
  ...
  void _showPlanDialog(BuildContext context, UserProfile user, DeveloperService service) {
  ...
  Color _getPlanColor(SubscriptionPlan plan) {
    switch (plan) {
      case SubscriptionPlan.free: return Colors.grey[300]!;
      case SubscriptionPlan.professional: return Colors.amber[200]!;
      case SubscriptionPlan.enterprise: return Colors.purple[200]!;
...
class _ActivationCodeTabState extends ConsumerState<ActivationCodeTab> {
  ...
  Future<void> _generateAndSaveCode() async {
    ...
    try {
      ...
      await ref.read(firebaseServiceProvider).createActivationCode(
            code: code,
                              ...
                              children: [
                                ...
                                SizedBox(
                                  ...
                                  child: ElevatedButton(
                                    onPressed: _isLoading ? null : _generateAndSaveCode,
                                    child: _isLoading
                     ...
                     builder: (context, snapshot) {
                         ...
                         return ListView.separated(
                             ...
                             itemBuilder: (context, index) {
                                 ...
                                 return ListTile(
                                     title: SelectableText(code.code, style: const TextStyle(fontFamily: 'Courier', fontWeight: FontWeight.bold)),
                                     subtitle: Column(
                                         crossAxisAlignment: CrossAxisAlignment.start,
                                         children: [
                                             Text('Duration: ${code.durationMonths} months • Created: ${DateFormat.yMMMd().format(code.createdAt)}'),
                                             if (isRedeemed) ...[
                                                 const SizedBox(height: 4),
                                                 Text(
                                                 ...
                                                 if (code.redeemedByEmail != null) Text('Email: ${code.redeemedByEmail}'),
                                                 if (code.redeemedByPhone != null) Text('Phone: ${code.redeemedByPhone}'),
                                             ]
                                         ],
                                     ),
                                     trailing: Row(
                                         ...
                                         children: [
                                             if (isRedeemed)
                                                const Chip(label: Text('USED'), backgroundColor: Colors.amberAccent)
                                             else
