...
class XRayViewer extends StatefulWidget {
  ...
  const XRayViewer({super.key, required this.xray, required this.patientName});
...
class _XRayViewerState extends State<XRayViewer> {
  ...
  void _zoomAtPoint(Offset localPoint, double scaleFactor) {
    ...
    final Matrix4 newMatrix = oldMatrix.clone()
      ...
      ..multiply(
        Matrix4.translationValues(-localPoint.dx, -localPoint.dy, 0.0),
      );
  ...
  Future<void> _saveCopy(BuildContext context, WidgetRef ref) async {
    ...
    try {
      ...
      final tempFile = File(
        '${tempDir.path}/temp_edited_xray_${DateTime.now().millisecondsSinceEpoch}.png',
      );
      ...
      if (context.mounted) {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text(l10n.saveCopySuccess)));
        }
      }
    } catch (e) {
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Failed to save copy: $e')));
      }
    }
  }
  ...
  Future<void> _editNotes(BuildContext context, WidgetRef ref) async {
    ...
    if (newNotes != null && newNotes != _currentNotes) {
      await ref
          .read(imagingServiceProvider)
          .updateXray(widget.xray.copyWith(notes: newNotes));
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(l10n.saveButton)));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Consumer(
      // Wrap in Consumer to access ref
                        ...
                        child: Column(
                          children: [
                            _buildSidebarButton(
                              ...
                              color: _currentNotes.isNotEmpty
                                  ? Colors.yellowAccent
                                  : null,
                            ...
                            _buildSidebarButton(
                              ...
                              onPressed: () => setState(
                                () => _activeFilter = _activeFilter == 'Sharpen'
                                    ? 'None'
                                    : 'Sharpen',
                              ),
                            ),
                            _buildSidebarButton(
                              ...
                              onPressed: () => setState(
                                () => _activeFilter = _activeFilter == 'Emboss'
                                    ? 'None'
                                    : 'Emboss',
                              ),
                            ),
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              Text(
                                AppLocalizations.of(context)!.negativeFilter,
                                style: const TextStyle(
                              ...
                              child: RepaintBoundary(
                                ...
                                child: Stack(
                                  ...
                                  children: [
                                    ...
                                    Positioned.fill(
                                      child: CustomPaint(
                                        painter: AnnotationDotsPainter(
                                          annotations: _textAnnotations,
                                        ),
                                      ),
                                    ),
                                    ..._textAnnotations.map(
                                      (t) => Positioned(
                                        left: t.offset.dx + 8,
                                        top: t.offset.dy - 10,
                                        child: Container(
  ...
  Widget _buildSidebarButton({
    ...
    return Padding(
      ...
      child: IconButton(
        icon: Icon(
          icon,
          color: isActive ? AppColors.primary : (color ?? Colors.white70),
        ),
        tooltip: tooltip,
        onPressed: onPressed,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var a in annotations) {
      canvas.drawCircle(
        ...
        5,
        paint,
      ); // 10px diameter, centered at offset
    }
  }
